...
use crate::{
    api_key::{ApiKey, ApiKeyNew, ApiKeyUpdate, ApiKeyUpdateInternal, HashedApiKey},
    errors, generics,
    schema::api_key::{self as model, dsl},
...
impl ApiKey {
    ...
    where
        ...
        O: Expression,
    {
        generics::generic_filter::<model::table, P, O, Self>(conn, predicate, limit, offset, order)
            .await
    }
    ...
    pub async fn update_by_id(
        ...
        match generics::generic_update_by_id::<<Self as HasTable>::Table, _, _, _>(
            conn,
            id.clone(),
            ApiKeyUpdateInternal::from(values),
        )
        .await
        {
